...
def Barbeiro(n):
    ...
    while True:
        if len(lista_espera)==0:
            estado_semaforo[n]-=1
            semaforo_barbeiro_durmir[n].acquire()
            if estado_barbeiro[n]==LIVRE:
                print("Barbeiro",n+1,"dormindo")
            else:
                print("Barbeiro",n+1,"acordou")
        else:
            semaforo_barbeiro[n].acquire()
            semaforo_cliente0.acquire()
